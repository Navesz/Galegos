...
function fieldClass(hasError: boolean, multiline = false) {
  return cn(
    "w-full rounded-xl border bg-brand-cream/50 px-4 text-sm text-brand-brown placeholder:text-brand-brown/35 outline-none transition-colors focus:border-brand-orange focus:ring-2 focus:ring-brand-orange/20",
    multiline ? "min-h-24 resize-none py-3" : "h-12",
...
export function CheckoutForm({
  ...
  }, [customer.cep, isDelivery, lookupCep]);

  const nameError = showErrors && customer.name.trim().length === 0;
  const cepErrorField =
    showErrors && isDelivery && onlyDigits(customer.cep).length !== 8;
  const streetError =
    showErrors && isDelivery && customer.street.trim().length === 0;
  const numberError =
    showErrors && isDelivery && customer.number.trim().length === 0;
  const neighborhoodError =
    showErrors && isDelivery && customer.neighborhood.trim().length === 0;
  const cityError =
    showErrors && isDelivery && customer.city.trim().length === 0;
  const stateError =
    showErrors && isDelivery && customer.state.trim().length !== 2;
  const paymentError =
    showErrors && isDelivery && customer.paymentMethod === "";
  ...
  return (
    <section className="mt-6 rounded-2xl border border-brand-brown/8 bg-white p-4 shadow-sm">
      <h2 className="text-base font-extrabold text-brand-brown">
        {isDelivery ? "Dados para entrega" : "Dados para retirada"}
      </h2>
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {ORDER_TYPE_OPTIONS.map((option) => {
              const selected = customer.orderType === option.id;

              return (
                <button
                  key={option.id}
                  type="button"
                  onClick={() => onChange({ orderType: option.id })}
                  className={cn(
                    "rounded-xl border py-3 text-sm font-bold transition-all active:scale-95",
                    selected
        ...
        <div>
          ...
          <textarea
            id="customer-notes"
            placeholder="Ex.: sem uva-passa, sem cebola, ponto da carne..."
            value={customer.notes}
            onChange={(event) => onChange({ notes: event.target.value })}
            className={fieldClass(false, true)}
          />
          <p className="mt-1 text-xs text-brand-brown/45">
            Opcional — use para tirar ingredientes ou pedir algo especial
          </p>
        </div>
